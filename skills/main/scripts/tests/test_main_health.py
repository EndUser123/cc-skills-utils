...
def test_env_audit_script_exits_critical_when_keys_missing():
    """End-to-end: env_audit.py must exit 2 and report missing keys."""
    import json as _json
    import subprocess as _sp
    import sys as _sys
    script = Path(__file__).parent.parent / "env_audit.py"
    if not script.exists():
        pytest.skip(f"env_audit.py not present at {script}")
    proc = _sp.run([_sys.executable, str(script)], capture_output=True, text=True, timeout=10)
    payload = _json.loads(proc.stdout)
    assert proc.returncode in (0, 2)
    assert payload["name"] == "env_audit"
    if proc.returncode == 2:
        assert payload["status"] == "critical"
        assert "missing" in payload["message"].lower() or "not found" in payload["message"].lower()


def test_env_audit_script_exits_critical_when_file_missing(tmp_path, monkeypatch):
    """End-to-end: when the .env path is missing, env_audit.py exits 2 with not-found message."""
    import json as _json
    import subprocess as _sp
    import sys as _sys
    script = Path(__file__).parent.parent / "env_audit.py"
    if not script.exists():
        pytest.skip(f"env_audit.py not present at {script}")
    # Run env_audit as a subprocess with cwd redirected to an empty tmp_path
    # so its relative P:/.env lookup still hits the real one (P:/.env is absolute
    # via Path, but test the path-missing branch in-process):
    sys.path.insert(0, str(script.parent))
    import env_audit  # type: ignore[import-not-found]
    monkeypatch.setattr(env_audit, "ENV_FILE_PATH", tmp_path / "no_such_file.env")
